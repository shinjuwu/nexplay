...
type Context struct {
	...
	isDebugMode  bool
	backendIp    string // 後台 ip
	*gin.Context        // gin object to be executed every time a api call is executed
}

func NewContext(_logger *logger.RuntimeGoLogger, _c *gin.Context, _db database.IDatabase, _rdb redis.IRedisCliect, _jwt *jwt.JwtManager,
	_captcha captcha.ICaptcha, _jobScheduler *job.JobScheduler, _ip string, _isDebugMode bool, _backendIp string) *Context {
...
func (c *Context) GetBackendIp() string {
	return c.backendIp
}
...
func (c *Context) Result(code int, msg string, data interface{}) {
	...
	c.logger.Info("RESULT %s %s%s, From:%s, RawQuery: %s, body: %s, code: %d, msg: %s, data: %v",
...
func (c *Context) OkWithCodeAndData(code int, data interface{}) {
	jsonData, _ := ioutil.ReadAll(c.Request.Body)
	c.logger.Info("CODE AND DATA %s %s%s, From:%s, RawQuery: %s, body: %s, code is %v, data: %v",
		c.Context.Request.Method,
		c.Context.Request.Host,
		c.Context.Request.RequestURI,
		c.ClientIP(),
		...
		code,
		data)
	response.Result(c.Context, code, "", data)
}

func (c *Context) OkWithCode(code int) {
	jsonData, _ := ioutil.ReadAll(c.Request.Body)
	c.logger.Info("CODE %s %s%s, From:%s, RawQuery: %s, body: %s, code is %v",
		c.Context.Request.Method,
		c.Context.Request.Host,
		c.Context.Request.RequestURI,
		c.ClientIP(),
...
func (c *Context) Ok(data interface{}) {
	jsonData, _ := ioutil.ReadAll(c.Request.Body)
	c.logger.Info("OK %s %s%s, From:%s, RawQuery: %s, body: %s, response is %v",
		c.Context.Request.Method,
		c.Context.Request.Host,
		c.Context.Request.RequestURI,
		c.ClientIP(),
...
func (c *Context) Fail(data interface{}) {
	jsonData, _ := ioutil.ReadAll(c.Request.Body)
	c.logger.Info("FAIL %s %s%s, From:%s, RawQuery: %s, body: %s, response is %v",
		c.Context.Request.Method,
		c.Context.Request.Host,
		c.Context.Request.RequestURI,
		c.ClientIP(),
...
func (c *Context) FailWithLogError(err error, data interface{}) {
	jsonData, _ := ioutil.ReadAll(c.Request.Body)
	c.logger.Error("ERROR %s %s%s, From:%s, RawQuery: %s, body: %s, error is %s",
		c.Context.Request.Method,
		c.Context.Request.Host,
		c.Context.Request.RequestURI,
		c.ClientIP(),
