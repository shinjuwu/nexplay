...
func (p *ChannelHandleRequest) Set(agent, agentCode, levelCode, timestamp, param, key, currency string) {
	p.Agent = agent
	p.AgentCode = agentCode
	p.LevelCode = levelCode
	p.TimeStamp = timestamp
	p.Param = param
	// p.ParamMap = make(map[string]interface{})
	p.Key = key
	p.Currency = currency
}

// assign & check
func (p *ChannelHandleRequest) Assign(agent, agentCode, levelCode, timestamp, param, key, aesKey, md5Key, currency string) (isSuccess bool, returnMsg string) {
	...
	*/
	...
	p.Set(agent, agentCode, levelCode, timestamp, param, key, currency)
	// 先用 base64 解密
