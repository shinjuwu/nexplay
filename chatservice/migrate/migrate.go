...
func StartUpCheck(logger *zap.Logger, db *sql.DB) {
	...
	if err != nil {
		logger.Fatal("Could not get migration records, run `chat service migrate up`", zap.Error(err))
	}
	...
	if diff < 0 {
		// update migrate file
		logger.Warn("DB schema newer, update chat service", zap.Int64("migrations", int64(math.Abs(float64(diff)))))
	}
}
