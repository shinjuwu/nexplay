  ...
  <div class="overflow-x-auto whitespace-nowrap">
    <div>
      ...
      >
        ...
        <template #betDetail>
          <span v-for="(betArea, betAreaIdx) in playerLog.betAreas" :key="`hundredsicboBetArea__${betArea.areaId}`">
            {{
              t('fmtTextBetAreaResult', [t(`hundredsicboBetArea__${betArea.areaId}`), numberToStr(betArea.betScore)])
            }}
    ...
    <div class="mt-4">
      ...
      <div v-for="winArea in gameLog.winAreas" :key="`hundredsicboBetArea__${winArea}`">
        {{ t(`hundredsicboBetArea__${winArea}`) }}
      </div>
    </div>
...
import { HundredsicboGameLog } from '@/base/common/gameLog/hundredsicbo'
...
const gameLog = computed(() => new HundredsicboGameLog(props.playLog))
</script>
