...
// ToFloat64 convert interface{} -> float64, if failed return default value
func ToFloat64(m interface{}, defaultValue ...float64) float64 {
	if val, ok := m.(float64); ok {
		return val
	}
	if len(defaultValue) > 0 {
		return defaultValue[0]
...
func StringToFloat64(m string, defaultValue ...float64) float64 {
