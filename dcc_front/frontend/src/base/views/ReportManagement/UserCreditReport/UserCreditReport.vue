            ...
            <table class="tbl tbl-hover">
              <thead>
                <tr>
                  ...
                  <PageTableSortableTh column="creditTime" :is-sort-icon-active="isSortIconActive" @sorting="sorting">
                  ...
                  <th>{{ t('textScoreChangeReason') }}</th>
                  <th>{{ t('textRoundId') }}&sol;{{ t('textBetId') }}</th>
                  <th>{{ t('textOperateResult') }}</th>
                  <th>{{ t('textSingleWalletId') }}&sol;{{ t('textJPTokenId') }}</th>
                </tr>
              </thead>
              <tbody>
                ...
                <template v-else>
                  <tr
                    ...
                    :key="`${record.agentId}_${record.userName}_${record.creditId}_${index}`"
                  >
                    <td>{{ record.agentName }}</td>
                    <td>{{ record.userName }}</td>
                    <td>{{ time.utcTimeStrToLocalTimeFormat(record.creditTime) }}</td>
                    <td>{{ record.beforeScore }}</td>
                    <td>{{ record.changeScore }}</td>
                    <td>{{ record.afterScore }}</td>
                    <td>{{ t(record.reasonI18nKey) }}</td>
                    <td>
                      <button
                        :class="{
                          'text-cyan-500 underline underline-offset-1': record.isCreditIdSupportShowSource,
                        }"
                        :disabled="!record.isCreditIdSupportShowSource"
                        @click="record.showCreditIdSourceFunc(record)"
                      >
                        {{ record.creditId }}
                      </button>
                    </td>
                    <td>
                      {{ record.resultI18nKeys.length > 0 ? record.resultI18nKeys.map((k) => t(k)).join('/') : '-' }}
                    </td>
...
import { useUserCreditReport } from '@/base/composable/reportManagement/userCreditReport/useUserCreditReport'
...
const { t } = useI18n()
const { isAdminUser, formInput, tableInput, records, searchRecords, downloadRecords } = useUserCreditReport()
</script>
