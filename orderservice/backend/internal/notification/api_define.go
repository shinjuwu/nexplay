...
var (
	//default
	ApiNotification_baseAddr = "http://172.30.0.154:9642/"
)
...
func GetGameServerGoldDetail(userid int64) (float64, float64, int, error) {
	...
	if code, ok := result["code"].(float64); ok {
		if code == 0 {
			// var goldFlag, lockGoldFlag bool
			if freeGold, ok := result["gold"].(float64); ok {
				// return gold, utils.ToInt(code), nil
				totlalGold += freeGold
				// goldFlag = ok
			}
...
*/
func SendSetKillDive(agnetId, gameId, roomType int64, killRate float64) (string, int, error) {
	...
	resMap["killdiveinfo"] = tmpArrayMap
