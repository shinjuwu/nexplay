...
func NewWebSocketClient(host, channel, rawQuery string) (*WebSocketClient, error) {
	conn := WebSocketClient{
		sendBuf: make(chan []byte, 1),
	}
	conn.ctx, conn.ctxCancel = context.WithCancel(context.Background())

	u := url.URL{Scheme: "ws", Host: host, Path: channel, RawQuery: rawQuery}
	...
	for {
		...
		case <-ticker.C:
			for {
				...
				messageType, bytMsg, err := ws.ReadMessage()
				if err != nil {
