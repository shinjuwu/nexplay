ALTER TABLE "public"."agent" 
  DROP COLUMN "child_agent_count",
  DROP COLUMN "jackpot_status",
  DROP COLUMN "jackpot_start_time",
  DROP COLUMN "jackpot_end_time";
...
DELETE FROM "public"."storage" WHERE "key" = 'JackpotSetting';

DELETE FROM "public"."permission_list" WHERE feature_code IN (100105, 100307, 100308, 100309, 100310, 100311, 100312, 100313, 100314, 100315, 100316, 100317);
...
FROM (
    ...
    WHERE "ag2ple"::int NOT IN (100307, 100308, 100309, 100310, 100311, 100312, 100313, 100314, 100315, 100316, 100317)
    GROUP BY "ag2"."id"
) AS "ag3"
WHERE "ag1"."id" = "ag3"."id";