...
type ICaptcha interface {
	GenerateCaptcha() (string, string, string, string, error)
...
// 依照傳入的類型產生不同種類的驗證碼
func (p *CaptchaDigit) GenerateCaptcha() (string, string, string, string, error) {
	expiredTimeStr := time.Now().Local().UTC().Add(p.ExpiredSec).String()
	id, b64s, answer, err := p.captchaDriverDigit.Generate()
	return id, b64s, answer, expiredTimeStr, err
}
