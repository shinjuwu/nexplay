...
func (r *RedisCliect) IncrWithExpire(dbIdx int, key string, expiration time.Duration) (int64, error) {
	if v, ok := r.redis_cliect.Load(dbIdx); ok {
		client := v.(*redis.Client)
...
*/
func (r *RedisCliect) LoadHValues(dbIdx int, hash string, fields []string) ([]interface{}, error) {
	if v, ok := r.redis_cliect.Load(dbIdx); ok {
		client := v.(*redis.Client)
		return client.HMGet(hash, fields...).Result()
	} else {
...
func (r *RedisCliect) HIncrBy(dbIdx int, hash string, field string, value int64) (int64, error) {
	if v, ok := r.redis_cliect.Load(dbIdx); ok {
		client := v.(*redis.Client)
		return client.HIncrBy(hash, field, value).Result()
	} else {
		return -1, errors.New("no register redis object")
	}
}

func (r *RedisCliect) HIncrByFloat(dbIdx int, hash string, field string, value float64) (float64, error) {
	if v, ok := r.redis_cliect.Load(dbIdx); ok {
		client := v.(*redis.Client)
		return client.HIncrByFloat(hash, field, value).Result()
	} else {
		return -1, errors.New("no register redis object")
	}
}
...
func (r *RedisCliect) LoadLValue(dbIdx int, key string, start, stop int64) ([]string, error) {
	if v, ok := r.redis_cliect.Load(dbIdx); ok {
		client := v.(*redis.Client)
...
func (r *RedisCliect) DeleteLValue(dbIdx int, key string, start, stop int64) (string, error) {
	if v, ok := r.redis_cliect.Load(dbIdx); ok {
		client := v.(*redis.Client)
		return client.LTrim(key, start, stop).Result()
	} else {
		return "", errors.New("no register redis object")
	}
}
