      ...
      <template #default="{ show }">
        <form v-show="show">
          <div class="flex flex-wrap items-center">
            ...
            <FormElectronicGameRoomTypeListDropdown v-model="formInput.roomType" class="mb-1 w-full md:w-3/12" />
          </div>
              ...
              <table class="tbl tbl-hover">
                ...
                <tbody class="text-slate-500">
                  <template v-if="currentRecords.length === 0">
                    <tr>
                      <td class="table-td text-center" colspan="9">{{ t('textTableEmpty') }}</td>
                    </tr>
                  </template>
                  <template v-else>
                    <tr v-for="(record, index) in currentRecords" :key="`electronicRecord_${index}`">
                      <td>{{ record.agentName }}</td>
                      <td>{{ record.gameCode }}</td>
                      <td>{{ t(`game__${record.gameId}`) }}</td>
                      <td>{{ t(`roomType__${roomTypeNameIndex(record.gameId, record.roomType)}`) }}</td>
                      <td>
                        <span v-if="record.isParent" class="text-danger">{{ t('textUsingGeneralAgentRTPSet') }}</span>
                        <span v-else>{{ numberToStr(record.killRatio * 100, 1) + '%' }}</span>
                      </td>
                      <td>
...
import FormElectronicGameRoomTypeListDropdown from '@/base/components/Form/Dropdown/FormElectronicGameRoomTypeListDropdown.vue'
