...
func NewPostgreDB(multiLogger *zap.Logger, config config.Config) IDatabase {
	...
	for idx, connInfo := range config.GetDatabase().ConnInfo {
		err := dbc.CreateDatabaseObject(idx, deiverName, connInfo)
		if err != nil {
			multiLogger.Fatal("Database create error", zap.Error(err))
		}
	}
...
// 創建一個 db 物件
func (p *PostgreDB) CreateDatabaseObject(dbIdx int, driverName, connInfo string) error {

	p.CheckDatabaseExist(dbIdx, driverName, connInfo)
 ...
 */
func (p *PostgreDB) CheckDatabaseExist(dbIdx int, driverName, connInfo string) {
	...
	if err = db.QueryRow("SELECT EXISTS (SELECT 1 from pg_database WHERE datname = $1)", dbname).Scan(&dBExists); err != nil {
		...
		if err != nil {
			...
			db.Close()
			log.Fatal("Failed to check if db exists", zap.String("db", dbname), zap.Error(err))
		}
	}

	if !dBExists {
		// Database does not exist, create it
		log.Println("Creating new database", zap.String("name", dbname))
	...
	if err = db.QueryRow("SELECT version()").Scan(&dbVersion); err != nil {
		db.Close()
		log.Fatal("Error querying database version", zap.Error(err))
	}
