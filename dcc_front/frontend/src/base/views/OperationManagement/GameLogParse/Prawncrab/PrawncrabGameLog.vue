  ...
  <div class="overflow-x-auto whitespace-nowrap">
    <div>
      ...
      >
        ...
        <template #betDetail>
          <span v-for="(betArea, betAreaIdx) in playerLog.betAreas" :key="`prawncrabBetArea__${betArea.areaId}`">
            {{ t('fmtTextBetAreaResult', [t(`prawncrabBetArea__${betArea.areaId}`), numberToStr(betArea.betScore)]) }}
    ...
    <div class="mt-4">
      ...
      <div>
        {{
          t(
            'fmtTextPrawncrabResult',
            gameLog.result.map((r) => t(`prawncrabResult__${r}`))
          )
        }}
      </div>
      <div v-for="winArea in gameLog.winAreas" :key="`prawncrabBetArea__${winArea}`">
        {{ t(`prawncrabBetArea__${winArea}`) }}
      </div>
    </div>
...
import { PrawncrabGameLog } from '@/base/common/gameLog/prawncrab'
...
const gameLog = computed(() => new PrawncrabGameLog(props.playLog))
</script>
