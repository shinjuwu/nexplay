...
// 依照key值,將計數加1,如果為第一次設定及設定過期時間
func (r *RedisCliectV9) IncrWithExpire(dbIdx int, key string, expiration time.Duration) (int64, error) {
	if v, ok := r.redis_cliect.Load(dbIdx); ok {
		client := v.(*redis.Client)
		count, err := client.Incr(r.ctx, key).Result()
		if err == nil && count == 1 {
			err = client.Expire(r.ctx, key, expiration).Err()
		}
		return count, err
	} else {
...
*/
func (r *RedisCliectV9) LoadHValues(dbIdx int, hash string, fields []string) ([]interface{}, error) {
	if v, ok := r.redis_cliect.Load(dbIdx); ok {
		client := v.(*redis.Client)
		return client.HMGet(r.ctx, hash, fields...).Result()
	} else {
...
func (r *RedisCliectV9) HIncrBy(dbIdx int, hash string, field string, value int64) (int64, error) {
	if v, ok := r.redis_cliect.Load(dbIdx); ok {
		client := v.(*redis.Client)
		return client.HIncrBy(r.ctx, hash, field, value).Result()
	} else {
		return -1, errors.New("no register redis object")
	}
}

func (r *RedisCliectV9) HIncrByFloat(dbIdx int, hash string, field string, value float64) (float64, error) {
	if v, ok := r.redis_cliect.Load(dbIdx); ok {
		client := v.(*redis.Client)
		return client.HIncrByFloat(r.ctx, hash, field, value).Result()
	} else {
		return -1, errors.New("no register redis object")
	}
}
...
func (r *RedisCliectV9) LoadLValue(dbIdx int, key string, start, stop int64) ([]string, error) {
	if v, ok := r.redis_cliect.Load(dbIdx); ok {
		client := v.(*redis.Client)
		return client.LRange(r.ctx, key, start, stop).Result()
	} else {
		return nil, errors.New("no register redis object")
	}
}

func (r *RedisCliectV9) DeleteLValue(dbIdx int, key string, start, stop int64) (string, error) {
	if v, ok := r.redis_cliect.Load(dbIdx); ok {
		client := v.(*redis.Client)
		return client.LTrim(r.ctx, key, start, stop).Result()
	} else {
		return "", errors.New("no register redis object")
	}
}
