  ...
  <div class="overflow-x-auto whitespace-nowrap">
    <div>
      ...
      >
        ...
        <template #betDetail>
          <span v-for="(betArea, betAreaIdx) in playerLog.betAreas" :key="`fantanBetArea__${betArea.areaId}`">
            {{ t('fmtTextBetAreaResult', [t(`fantanBetArea__${betArea.areaId}`), numberToStr(betArea.betScore)]) }}
    ...
    <div class="mt-4">
      ...
      <div v-for="winArea in gameLog.winAreas" :key="`fantanBetArea__${winArea}`">
        {{ t(`fantanBetArea__${winArea}`) }}
      </div>
    </div>
...
import { FantanGameLog } from '@/base/common/gameLog/fantan'
...
const gameLog = computed(() => new FantanGameLog(props.playLog))
</script>
